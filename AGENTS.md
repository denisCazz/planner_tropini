...
This is **Planner Bitora**, a single Next.js 16 (App Router) + Prisma + PostgreSQL field-sales CRM (Italian UI). Package manager is **npm**. Standard scripts live in `package.json` (`dev`, `build`, `start`, `lint`).

Service / DB notes (non-obvious):
- **PostgreSQL is required.** The cloud VM runs a local Postgres 16 cluster. If it is not running, start it with `sudo pg_ctlcluster 16 main start`. The dev DB is `planner_Bitora` (user/password `postgres`/`postgres`).
- **`DATABASE_URL` is set in a git-ignored `.env`** at the repo root (Prisma reads it). The update script does not create `.env`, so if `.env` is missing, recreate it: `DATABASE_URL="postgresql://postgres:postgres@localhost:5432/planner_Bitora?schema=public"`.
